 ...
 **************************************************************/
 ...
 pub fn calculate_heat_index(t: f32, rh: f32) -> f32 {
    let temp_f = (t * 1.8) + 32.0;

    /* calculate simple equation first */
    let mut hi_temp = 0.5 * ( temp_f + 61.0 + ( 1.2 * ( temp_f - 68.0 ) ) + ( rh * 0.094 ) );

    let avg = ( hi_temp + temp_f ) / 2.0; 
    
    let hi: f32 = 
        if avg >= 80.0 {
            /* perform regression */
            let a = -42.379 + ( 2.04901523 * temp_f ) + ( 10.14333127 * rh );
            let b = -0.22475541 * temp_f * rh - 0.00683783 * powf( temp_f, 2.0 );
            let c = -0.05481717 * powf(rh, 2.0) + 0.00122874 * powf(temp_f, 2.0) * rh;
            let d = 0.00085282 * temp_f * powf(rh, 2.0) - 0.00000199 * powf(temp_f, 2.0) * powf(rh, 2.0);
